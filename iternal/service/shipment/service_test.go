...
func TestSqlShipmentService_GetById(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, barcode, sender, receiver, is_delivered, origin, destination, created_at FROM shipment WHERE id = .*").
		WillReturnRows(rows).WithArgs(1)
	service := NewSQLShipmentService(db, &bloom.Filter{})
	s, err := service.SelectShipmentById(1)
	checkEquals(s, t)
}
...
func TestSqlShipmentService_GetByBarcode(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "barcode", "sender", "receiver", "is_delivered", "origin", "destination", "created_at"}).
		AddRow(1, "TESTCODE1003", "Test", "Test", false, "Test", "Test", time.Time{})
	mock.ExpectQuery("SELECT id, barcode, sender, receiver, is_delivered, origin, destination, created_at FROM shipment WHERE barcode = .*").
		WillReturnRows(rows).WithArgs("TESTCODE1003")
	service := NewSQLShipmentService(db, &bloom.Filter{})
	s, err := service.SelectShipmentByBarcode("TESTCODE1003")
	checkEquals(s, t)
}
...
func TestSqlShipmentService_DeleteById(t *testing.T) {
	...
	mock.ExpectExec("DELETE FROM shipment WHERE id = .*").WithArgs(1).WillReturnResult(sqlmock.NewResult(1, 1))

	service := NewSQLShipmentService(db, &bloom.Filter{})
	err = service.DeleteShipmentById(1)
...
func TestSqlShipmentService_Save(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO shipment").WithArgs(
		shipment.Barcode,
		shipment.Sender,
		shipment.Receiver,
	...
	err = service.InsertShipment(shipment)
	if err != nil {
...
func checkEquals(s *model.Shipment, t *testing.T) {
	expectedShipment := &model.Shipment{
		Id:          1,
